...
public class JTableUtils {
    ...
    /**
     * Запись данных из массива Boolean[] в JTable
     * (основная реализация, закрытый метод, используется в ArrayToGrid и Array2ToGrid)
     */
    ...
    /**
     * Чтение данных из JTable в двухмерный массив Boolean
     * (основная реализация, используется в остальных readBArrayFromJTable и readBMatrixFromJTable)
     */
    public static <T> T[][] readBMatrixFromJTable(
            JTable table, Class<T> clazz, Function<Boolean, ? extends T> converter,
            boolean errorIfEmptyCell, T emptyCellValue
    ) throws JTableUtilsException {
        TableModel tableModel = table.getModel();
        int rowCount = tableModel.getRowCount(), colCount = tableModel.getColumnCount();
        ...
        for (int r = 0; r < rowCount; r++) {
            for (int c = 0; c < colCount; c++) {
                ...
                if (obj == null || obj instanceof String && ((String) obj).trim().length() == 0) {
                    ...
                    value = converter.apply(Boolean.valueOf(obj.toString()));
                }
                matrix[r][c] = value;
            }
        }
    ...
    /**
     * Чтение данных из JTable в одномерный массив Boolean
     */
    public static <T> T[] readBArrayFromJTable(
            JTable table, Class<T> clazz, Function<Boolean, ? extends T> converter,
            boolean errorIfEmptyCell, T emptyCellValue
    ) throws JTableUtilsException {
        T[][] matrix = readBMatrixFromJTable(table, clazz, converter, errorIfEmptyCell, emptyCellValue);
        return matrix[0];
    }
    /**
     * Чтение данных из JTable в одномерный массив
    ...
    public static <T> T[] readBArrayFromJTable(
            JTable table, Class<T> clazz, Function<Boolean, ? extends T> converter, T emptyCellValue
    ) {
        try {
            return readBArrayFromJTable(table, clazz, converter, false, emptyCellValue);
        } catch (JTableUtilsException impossible) {
        }
        return null;
    }
    ...
    public static <T> T[] readBArrayFromJTable(
        ...
        return readBArrayFromJTable(table, clazz, converter, true, null);
    }
    ...
    public static Boolean[] readBooleanArrayFromJTable(JTable table) {
        try {
            return readBArrayFromJTable(table, Boolean.class, (b) -> b, false, false);
        } catch (JTableUtilsException impossible) {
        }
        return null;
    }
